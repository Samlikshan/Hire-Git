...
const EditPlanModal: React.FC<EditPlanModalProps> = ({
  ...
  const handleInputChange = (
    ...
    if (type === "checkbox") {
      ...
      setEditedPlan({ ...editedPlan, [name]: target.checked });
    } else if (type === "number") {
      setEditedPlan({ ...editedPlan, [name]: parseFloat(value) });
    } else {
  ...
  const addFeature = () => {
    if (tempFeature.key.trim() && tempFeature.value.trim()) {
      setEditedPlan({
        ...editedPlan,
        features: {
          ...editedPlan.features,
          ...
          <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {/* <div>
                  ...
                  <input
                    ...
                    min="0"
                    step="0.01"
                    value={editedPlan.annualPrice}
                    onChange={handleInputChange}
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-indigo-500 focus:border-indigo-500"
                  />
                  {editedPlan.monthlyPrice > 0 &&
                    editedPlan.annualPrice > 0 && (
                      <div className="mt-1 text-sm text-green-600">
                        Annual savings: $
                        {(
                          editedPlan.monthlyPrice * 12 -
                          editedPlan.annualPrice
                        ).toFixed(2)}
                      </div>
                    )}
                </div> */}
            ...
            <div className="pt-4 space-y-4">
              ...
              <div className="space-y-3">
                ...
                {Object.entries(editedPlan.features).map(
                  ([key, value], index) => (
